...
with patch.object(requests.Session, 'request', side_effect=AssertionError('Network disabled')), patch.object(config, 'get_archivebate_credentials', return_value=('', '')):
    ...
    original={'id':'1','source':'archivebate'}
    assert camwhores.deduplicate_videos([original, {'id':'1','source':'archivebate','title':'title'}])[0]['title']=='title'
    assert 'title' not in original
    samples=[]
    for count in (0,24,280):
        for group in ('0','1'):
            for _ in range(20):
                with patch.object(main, 'read_json_cache', return_value=(cards(count),time.time())), patch.object(main, 'get_catalog_stats', return_value={'last_page': 1, 'total_videos': count, 'base_catalog_videos': count, 'blocked_videos': 0, 'accuracy': 'exact'}), patch.object(main, '_published_catalog_revision', return_value=None), patch.object(main, '_available_catalog_revision', return_value=None), patch.object(main, '_ensure_catalog_indexing', return_value=None), patch.object(main, '_fetch_and_cache_home', side_effect=AssertionError('Blocking fetch')), patch.object(main.scraper, 'get_home_videos', side_effect=AssertionError('Blocking topup')), patch.object(main, '_enrich_videos', side_effect=lambda v,**kw:v[:24]):
                    start=time.perf_counter()
