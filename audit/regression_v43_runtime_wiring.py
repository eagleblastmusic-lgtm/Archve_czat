...
assert 'from fast_storyboard_quick import install' in runtime
assert 'install_parallel_quick_storyboard()' in runtime
assert 'parallel_quick_storyboard' in runtime
assert 'RUNTIME_ID = "v4.3-fast2"' in runtime
assert '/api/runtime/v43' in runtime
assert 'X-Archivebate-Runtime' in runtime
assert 'QUICK_PARALLELISM = 4' in quick
assert '_build_variant_parallel' in quick
...
print('PASS V4.3 RUNTIME WIRING + PARALLEL QUICK STORYBOARD')
